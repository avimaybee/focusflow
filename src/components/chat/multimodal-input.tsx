...
// Import existing UI components for persona selection
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
// Import lucide-react icons for personas
import {
  Users,
  Check,
  Bot,
...
// Button variants using cva - ADAPTED FOR FOCUSFLOW AI PALETTE
  ...
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        ghost: 'text-muted-foreground hover:bg-muted hover:text-foreground', // Explicitly set text to black
        // Link: black text
...
// Textarea component - ADAPTED FOR FOCUSFLOW AI PALETTE
  ...
  return (
    <textarea
      className={cn(
        ...
        'flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-base ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm text-foreground', // Added text-black
        className,
      )}
      ref={ref}
      {...props}
    ...
    return (
      ...
      >
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M8.70711 1.39644C8.31659 1.00592 7.68342 1.00592 7.29290 1.39644L2.21968 6.46966L1.68935 6.99999L2.75001 8.06065L3.28034 7.53032L7.25001 3.56065V14.25V15H8.75001V14.25V3.56065L12.7197 7.53032L13.25 8.06065L14.3107 6.99999L13.7803 6.46966L8.70711 1.39644Z"
          fill="currentColor"
        />
      </svg>
    );
...
function PureStopButton({ onStop }: { onStop: () => void }) {
  return (
    <Button
      ...
      className="rounded-full p-1.5 h-fit bg-primary text-primary-foreground"
      onClick={(event) => {
        event.preventDefault();
        onStop();
      }}
...
function PureSendButton({
  ...
  return (
    <Button
      ...
      className="rounded-full p-1.5 h-fit bg-primary text-primary-foreground disabled:bg-muted disabled:text-muted-foreground"
      onClick={(event) => {
        event.preventDefault();
        if (!isDisabled) {
          submitForm();
