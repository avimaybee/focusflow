...
export function ChatMessageSkeleton({ persona }: ChatMessageSkeletonProps) {
    ...
    return (
        <div className="group flex w-full gap-3 sm:gap-4 px-4 sm:px-6 py-0 sm:py-0.5 mb-1.5 sm:mb-2 last:mb-0 justify-start">
            <div className="flex flex-col gap-0.5 max-w-2xl sm:max-w-3xl items-start w-full">
                {persona?.name && (
                    <div className="flex items-center gap-1 px-1">
                        <p className="text-xs font-medium text-foreground/60">{persona.name}</p>
                    </div>
                )}
                <div className={`relative w-full text-sm sm:text-[15px] leading-relaxed text-foreground/90 border-l-2 pl-4 pr-3 py-1 sm:py-1.5 ${personaColorClass}`}>
